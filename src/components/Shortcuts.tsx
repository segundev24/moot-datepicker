...
const ItemTemplate = React.memo((props: ItemTemplateProps) => {
    const { period, changePeriod, updateFirstDate, dayHover, changeDayHover } =
        useContext(DatepickerContext);

    // Functions
    const getClassName: () => string = useCallback(() => {
        return "whitespace-nowrap w-1/2 md:w-1/3 lg:w-auto transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 p-2 rounded cursor-pointer currentColor hover:currentColor-800";
    }, []);
    ...
    const chosePeriod = useCallback(
        ...
        [changeDayHover, changePeriod, dayHover, period.end, period.start, updateFirstDate]
    );
...
const Shortcuts: React.FC = () => {
    ...
    return shortcutOptions?.length ? (
        <div className="w-[150px] md:border-b mb-3 lg:mb-0 lg:border-r lg:border-b-0 border-gray-300 dark:border-gray-700 pr-1">
